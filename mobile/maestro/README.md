...
1. **Emulator running** (Pixel 7 or similar). Claude-spawned emulators do not
   ...
   ```
2. **Dev build installed** on that emulator (`com.tracknotes.app`). The flows
   deep-link the Expo dev client, so a Go/Expo-Go install will not work.
...
own via `takeScreenshot`).

## Flows

| Flow | Purpose | Verified |
| --- | --- | --- |
| `app-launch.yaml` | Launch → "My Guides" renders. | Previously green (pre-2026-08 local runs) |
| `plan-screen.yaml` | Guide → Plan: inputs card, summary, day splits, water carries. | Previously green (pre-2026-08 local runs) |
| `guide-list.yaml` | My Guides lists all six bundled trails, badges read "Not downloaded", list scrolls. | **Unverified** — written 2026-08-19, no emulator/Metro available |
| `view-map.yaml` | Open a guide → map pane chrome (status pill, map key, FABs) mounts and survives pans + recenter. | **Unverified** — same |
| `toggle-views.yaml` | Map → Elevation → List segmented switching, plus the List filter chips. | **Unverified** — same |
| `waypoint-detail.yaml` | List → waypoint row → detail: name, stats, favorite round-trip, comments area. | **Unverified** — same |
| `add-comment-offline.yaml` | Post a comment with the API unreachable; assert the queued/failed affordance. | **Unverified** — same |
| `shared/launch-dev.yaml` | Launcher fragment — `clearState` + dev-client deep link. Not standalone. | — |

"Unverified" means: the YAML parses and every selector was read out of the
current app source, but no flow has been executed against a device. Maestro has
no offline linter (`maestro test` selects a device before parsing the flow), so
the first real run may need selector tweaks.
...
- **"Offline" in `add-comment-offline.yaml` is not airplane mode.** It relies on
  the default dev state: `EXPO_PUBLIC_API_BASE_URL` points at `localhost`, and
  inside the emulator `localhost` is the emulator itself (the host would be
  `10.0.2.2`), so requests are refused → `NetworkError`. The flow asserts both
  legal outcomes:
  - *no device identity yet* (default, since `clearState` wipes the SecureStore
    session): registration is the one hard network dependency in the comment
    path, so the display-name prompt stays open with "Couldn't reach the
    server…" and a "Try again" button, draft intact;
