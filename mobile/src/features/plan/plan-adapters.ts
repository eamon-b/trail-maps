...
export function computePlan(trail: TrailJson, inputs: PlanInputs): PlanResult {
  ...
  return {
    ...
    effectiveDailyKm,
    days,
    ...computePlanExtras(trail, section, effectiveDailyKm),
  };
}

/** The parts of a plan that do not depend on WHERE the day boundaries fall. */
export type PlanExtras = Pick<
  PlanResult,
  'resupply' | 'foodCarries' | 'water' | 'topWaterCarries'
>;
 ...
 *
 * @param effectiveDailyKm the realized km/day of whatever split is being
 *   shown (`sectionKm / days.length`), which is what turns a gap into "≈ N
 *   days" and a food weight.
 */
export function computePlanExtras(
  ...
  return { resupply, foodCarries, water, topWaterCarries };
}
