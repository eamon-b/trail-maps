...
// can be unit-tested without a DOM or a map.
...
interface Trail {
  ...
  /**
   * Uncurated OpenStreetMap points of interest near the corridor. Absent when
   * the trail was never enriched — which is not the same as "enriched and found
   * nothing", so an absent list hides the whole control rather than showing an
   * empty one.
   */
  pois?: TrailPOI[];
}
...
//
// A second, uncurated layer over the same trail: OSM taps, campsites, shops,
// bus stops and clinics found near the corridor. It is opt-out-able down to the
// category because the whole point is to *judge* the data — a walker who finds
// the water POIs useful and the transport ones noise can say so, and the choice
// survives a reload.
//
// Everything POI is deliberately additive: no leg, filter summary or CSV figure
// changes because POIs are on screen, and no POI row carries a hook the
// datasheet's delegated click handler matches on.

let poiFilter: PoiFilterState = loadPoiFilterState();
let poiLayer: L.LayerGroup | null = null;
let poiMarkers: Array<{ poi: TrailPOI; marker: L.Marker }> = [];
/** The control element, created lazily above the datasheet. */
let poiControl: HTMLElement | null = null;
/**
 * POIs handed to `setTrailPois` before the viewer had a trail — the imported
 * trail page enriches in the background and can finish either side of boot.
 */
let pendingPois: TrailPOI[] | null = null;

function currentPois(): TrailPOI[] {
  return trailState.currentTrail?.pois ?? [];
}

function createPoiIcon(category: string): L.DivIcon {
  return L.divIcon({
    className: `poi-marker poi-cat-${isPoiCategory(category) ? category : 'other'}`,
    html: poiCategoryIcon(category),
    iconSize: [16, 16],
    iconAnchor: [8, 8],
    popupAnchor: [0, -8],
  });
...
/**
 * Rebuild the POI marker layer from the current filter.
 *
 * One `L.layerGroup` rather than loose markers so the whole layer can be
 * dropped in a single call, and `zIndexOffset` keeps every POI beneath the
 * waypoint markers — a curated waypoint must never end up hidden behind an OSM
 * node that happens to share its coordinates.
 */
function drawPoiMarkers(): void {
  if (poiLayer && map && map.hasLayer(poiLayer)) map.removeLayer(poiLayer);
  poiLayer = null;
  poiMarkers = [];
  if (!map) return;

  const pois = visiblePois(currentPois(), poiFilter);
...
/** The POI row's "show on map": pan, open the popup, scroll the map into view. */
function showPoiOnMap(key: string): void {
  const found = poiMarkers.find(entry => poiKey(entry.poi) === key);
  if (!found || !map) return;
  map.setView([found.poi.lat, found.poi.lon], Math.max(map.getZoom(), 14));
  found.marker.openPopup();
  document.getElementById('trail-map')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
}

/**
 * The control's host element, created on first use.
 *
 * Built here rather than in page markup because the viewer serves three
 * different documents — the generated trail pages, `my-trail.html` and the
 * tests' fixtures — and only the first of those is regenerated by the build. A
 * JS-created host means POIs light up everywhere without a rebuild.
 *
 * The listener is bound once, to the host, because the control's innards are
 * replaced wholesale on every re-render (same reason the datasheet delegates
 * from `#waypoints-container`).
 */
function poiControlHost(): HTMLElement | null {
  if (poiControl?.isConnected) return poiControl;
  ...
  host.id = 'poi-control';
  host.className = 'poi-control';
  host.hidden = true;
  container.parentNode.insertBefore(host, container);

  host.addEventListener('change', (e) => {
    const input = e.target as HTMLInputElement;
    const category = input.dataset.poiCategory;
    if (input.id === 'poi-enabled') {
      poiFilter = { ...poiFilter, enabled: input.checked };
    } else if (category && isPoiCategory(category)) {
      poiFilter = {
        ...poiFilter,
        categories: { ...poiFilter.categories, [category]: input.checked },
      };
    } else {
      return;
    }
    savePoiFilterState(poiFilter);
    refreshPois();
  });
...
function renderPoiControl(): void {
  const host = poiControlHost();
  if (!host) return;
  ...
  host.hidden = false;
  host.innerHTML = poiControlHtml(pois, poiFilter);
}

/** Re-render every POI surface: the control, the map layer and the datasheet. */
function refreshPois(): void {
  renderPoiControl();
  drawPoiMarkers();
 ...
 * lookup, hand the results here, and the map layer, the control and the
 * datasheet rows all update in place. Safe to call before `initTrailViewer`
 * (the list is held and applied at boot) and safe to call repeatedly.
 */
export function setTrailPois(pois: TrailPOI[]): void {
...
function renderWaypoints(waypoints: Waypoint[] | undefined, alternates: RouteVariant[] | undefined, sideTrips: RouteVariant[] | undefined, offTrailWaypoints?: OffTrailWaypoint[]): void {
  ...
  // OSM points of interest, interleaved by trail km but structurally inert:
  // they are not in `computeWaypointLegs`, so no leg, no summary line and no
  // CSV figure moves because a POI is on screen, and their rows carry none of
  // the hooks the delegated click handler matches on.
  //
  // Under a family filter only the matching POI family stays, for the same
  // reason the waypoint rows are filtered — a "where is water?" reading should
  // not be broken up by bus stops.
  const poiRows = visiblePois(trailState.currentTrail?.pois, poiFilter)
    .filter(poi => !isFiltered || poi.category === filter);
  const interleaved = interleavePoisByDistance(tableRows, poiRows, row => row.distance);
  ...
  const tableHtml = `
    <table class="waypoints-table">
      ...
      <tbody>
        ${interleaved.map(entry => {
          if (entry.kind === 'poi') return poiRowHtml(entry.poi);
    ...
    ${poiRows.length > 0 ? `
      <p class="poi-table-attribution">
        Rows badged <span class="poi-source-badge">OSM</span> are uncurated points of interest.
        ${escapeHtml(OSM_ATTRIBUTION)}.
      </p>
    ` : ''}
  `;
...
export async function initTrailViewer(
  ...
  // Enrichment that finished before boot did.
  if (pendingPois) {
    trail.pois = pendingPois;
    pendingPois = null;
  }
  ...
  // The map layer is already up (initMap); this adds the control above the
  // datasheet, or leaves it absent when the trail carries no POIs.
  renderPoiControl();
  ...
  waypointsContainer?.addEventListener('click', (e) => {
    ...
    // POI rows are not expandable — their only interaction is this link.
    // Checked first so it can never fall through to the row matching below.
    if (target.classList.contains('poi-show-on-map')) {
      e.preventDefault();
      showPoiOnMap(target.dataset.poiKey ?? '');
      return;
    }
